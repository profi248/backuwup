...
use shared::types::{ClientId, PackfileId, SessionToken};

use crate::{net_p2p::get_ws_config, KEYS, UI, log};
...
impl BackupTransportManager {
    pub async fn new(addr: String, session_nonce: TransportSessionNonce, client_id: ClientId) -> anyhow::Result<Self> {
    ...
    pub fn parse_incoming_ack(data: &[u8], nonce: TransportSessionNonce, sender_pubkey: ClientId, messages_received: &mut u64) -> anyhow::Result<u64> {
    ...
    pub async fn process_acks(mut stream: SplitStream<WebSocketStream<MaybeTlsStream<TcpStream>>>, notifier: Sender<u64>, nonce: TransportSessionNonce, sender_pubkey: ClientId) {
        let mut messages_received: u64 = 0;
        while let Some(msg) = stream.next().await {
            match msg {
                Ok(Message::Binary(data)) => match Self::parse_incoming_ack(&data, nonce, sender_pubkey, &mut messages_received) {
                    Ok(message_id) => { notifier.send(message_id).ok(); }
                    Err(e) => { log!("[p2p] error while processing ack: {}", e); }
                },
    ...
    pub async fn send_data(&mut self, data: Vec<u8>, id: PackfileId) -> anyhow::Result<()> {
        ...
        self.tx.send(Message::Binary(msg)).await?;

        self.wait_for_ack(self.msg_counter).await?;
